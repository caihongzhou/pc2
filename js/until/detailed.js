getDatas.controller('detailed', ['$scope', '$http', '$state', function($scope, $http, $state) {
	//跳转修改页面
	...
	$scope._chengeno = function() {
		...
		if($scope._licenseNo && $scope._modelName && $scope._frameNo && $scope._engineNo) {
			...
			if($scope._totalPremium) {
				window.location.href = "./getData.html#/newbijia";
			} else {

				window.location.href = "./getData.html#/bijia";
			}
...
getDatas.controller('basic', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
	...
	$scope.Create = function() {
		...
		if($scope._cityCode && $scope._licenseNo && $scope._custName && $scope._phoneNo) {
			...
			Orderajax.setorder('getData.html#/sea', userId, $scope._licenseNo, $scope._custName, $scope._cappId, $scope._phoneNo, $scope._cityCode, $scope._orderId)
				//userId,licenseNo,ownerName,ownerID,ownerMobile,cityCode,orderId
		}
	}

}]);
getDatas.controller('basic_car', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
	//修改带出信息
	var userId = localStorage.getItem("userId");
	var orderId = localStorage.getItem("chenge_orderId");
	...
	$scope.upcarInfo = function() {
			...
			if($scope._licenseNo && $scope._modelName && $scope._frameNo && $scope._engineNo) {
				...
				setTimeout(function() {
					Orderajax.upCarInfo('getData.html#/sea', userId, orderId, $scope._licenseNo, $scope._frameNo, $scope._engineNo, $scope._modelCode, $scope._modelName, $scope._registerDate, $scope.yes, $scope._specialCarDate, $scope._customerId, $scope._carId, $scope._seatCount, $scope._newCarPrice, $scope._licenseDate)
...
getDatas.controller('basic_chengbao', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
	//修改带出信息
	$scope.sures = function() {
	...
	$scope.srder = function() {
		...
		Orderajax.updateKindCode(jisons, "getData.html#/Insurance");
		Orderajax.getCompanys(userId, $scope._carId, $scope._cityCode);
		localStorage.setItem("chenge_orderId", orderId);
	}
}]);
getDatas.controller('comparison', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
	...
	$scope.toubao = function(item, data) {
		...
		Orderajax.chooseInsurance("getData.html#/sea", orderId, item[0].precisionPriceId, item[0].biStartDate, item[0].ciStartDate)
	}
}]);
getDatas.controller('comparison_2', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
...
getDatas.controller('basic_applic', ['$scope', '$http', '$state', '$filter', function($scope, $http, $state, $filter) {
	//修改带出信息
	var userId = localStorage.getItem("userId");
	var orderId = localStorage.getItem("chenge_orderId");
	$http({
	...
	$scope.applicant = function() {
		...
		$('.flogbox p').css('display', 'none');
		$('.texta').text("请求中，请稍候...");
		console.log(userId, orderId, applicantAsOwner, $scope._applicantName, $scope._applicantIdNo, $scope._applicantMobile, insuredAsOwner, $scope._insuredName, $scope._insuredIdNo, $scope._insuredMobile, insuredAsAddress, $scope._insuredAddress, $scope._gift, $scope._giftType, $scope._addresseeName, $scope._addresseeMobile, $scope._addresseeDetails, addresseeCounty, addresseeCity, addresseeProvince, $scope._policyEmail, $scope._customerId, $scope._addressAsGift, $scope._giftAddresseeDetails, $scope._giftAddresseeName, $scope._giftAddresseeMobile, $scope._giftAddresseeCounty, $scope._giftAddresseeCity, $scope._giftAddresseeProvince)
...
getDatas.filter("codelfilter", function() {
...
getDatas.filter("amountfilter", function() {
	return function(code) {
		switch(code) {
			case 'Y':
				return '投保'
...
getDatas.filter("flage", function() {
	return function(code) {
		switch(code) {
			case '1':
				return '国产玻璃'
